  ...
  public func defaultConfigureAudioSessionFunc(newState: State, oldState: State) {
    if UIApplication.shared.applicationState == .active {
      DispatchQueue.liveKitWebRTC.async { [weak self] in
        self?.configuringDefaultAudioSessionProcess(newState: newState, oldState: oldState)
      }
    } else {
...
private extension AudioManager {
  func configuringDefaultAudioSessionProcess(newState: State, oldState: State) {
    // prepare config
